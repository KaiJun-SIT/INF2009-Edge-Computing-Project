...
# Get the absolute path and check file existence
script_dir = os.path.dirname(os.path.abspath(__file__))  # Get script directory
map_file = os.path.join(script_dir, "..", "data", "calib_generate_params_py.xml")
print(f"Attempting to open file at: {os.path.abspath(map_file)}")

# Check if file exists
if not os.path.exists(map_file):
    print(f"Error: File does not exist at path: {map_file}")
    exit()

# Try to open the file
cv_file = cv2.FileStorage(map_file, cv2.FILE_STORAGE_READ)
if not cv_file.isOpened():
    print(f"Error: Could not open file: {map_file}")
    exit()
print("read success")
